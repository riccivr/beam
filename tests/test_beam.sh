...
TOKEN=$(echo "$LINK_LINE" | sed -n 's/.*\/s\/\([a-f0-9]*\).*/\1/p')
...
# 5. HTML5 Player endpoint check
HTML_RESP=$(curl -s "http://127.0.0.1:$TEST_PORT/s/$TOKEN")
case "$HTML_RESP" in
    *"<video"*sample.mp4*) echo "[PASS] HTML5 player served with <video tag and filename" ;;
    *) echo "[FAIL] HTML5 player response unexpected:\n$HTML_RESP"; kill $BEAM_PID 2>/dev/null || true; exit 1 ;;
esac

# 6. Raw file download check
curl -s "http://127.0.0.1:$TEST_PORT/s/$TOKEN/raw" > "$TMP_DIR/downloaded.mp4"
...
curl -s -H "Range: bytes=0-9" -i "http://127.0.0.1:$TEST_PORT/s/$TOKEN/raw" > "$TMP_DIR/range1.out"
...
curl -s -H "Range: bytes=50-99" -i "http://127.0.0.1:$TEST_PORT/s/$TOKEN/raw" > "$TMP_DIR/range2.out"
...
curl -s -H "Range: bytes=-100" -i "http://127.0.0.1:$TEST_PORT/s/$TOKEN/raw" > "$TMP_DIR/range3.out"
...
STATUS_416=$(curl -s -o /dev/null -w "%{http_code}" -H "Range: bytes=999999-9999999" "http://127.0.0.1:$TEST_PORT/s/$TOKEN/raw")
...
HEAD_RESP=$(curl -s -I "http://127.0.0.1:$TEST_PORT/s/$TOKEN/raw")
...
./beam -q -P 9873 -1 -r "$TEST_TXT" > "$ONESHOT_LOG" 2>&1 &
BEAM_1_PID=$!
sleep 0.5
ONESHOT_TOKEN=$(grep "Link:" "$ONESHOT_LOG" | sed -n 's/.*\/s\/\([a-f0-9]*\).*/\1/p')

curl -s "http://127.0.0.1:9873/s/$ONESHOT_TOKEN/raw" > /dev/null
sleep 0.5
